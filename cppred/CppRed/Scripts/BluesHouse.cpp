...
DECLARE_SCRIPT(BluesHouseText1){
	...
	if (vs.get(EventId::event_got_town_map)){
		parameters.game->run_dialogue_from_world(TextResourceId::DaisyUseMapText, *parameters.caller);
	}else if (vs.get(EventId::event_got_pokedex)){
		parameters.game->run_dialogue_from_world(TextResourceId::DaisyOfferMapText, *parameters.caller);
		//TODO: Give map.
		vs.set(VisibilityFlagId::hs_town_map, false);
		parameters.game->run_dialogue_from_world(TextResourceId::GotMapText, *parameters.caller);
		vs.set(EventId::event_got_town_map, true);
	}else{
		parameters.game->run_dialogue_from_world(TextResourceId::DaisyInitialText, *parameters.caller);
	}
}
